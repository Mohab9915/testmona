    ...
    <div className="space-y-4">
      ...
      {parsed.blocks.map((block, index) => (
        <section key={`${block.type}-${block.title}-${index}`} className="rounded-md border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <h4 dir="auto" className="mt-3 wrap-break-word text-base font-semibold text-slate-950 dark:text-white">{block.title}</h4>
          ...
          <ol className="mt-3 space-y-1.5">
            {block.steps.map((step, stepIndex) => (
              <li key={`${step.keyword}-${stepIndex}`} className="flex flex-col gap-2 rounded-md bg-slate-50 p-2.5 text-sm dark:bg-slate-950/60 sm:flex-row sm:items-start sm:gap-3">
                ...
                <span dir="auto" className="min-w-0 wrap-break-word leading-6 text-slate-700 wrap-anywhere dark:text-slate-300">
                  {step.text}
                  {step.argument.length > 0 && (
                    <pre dir="auto" className="mt-2 overflow-x-auto rounded-md border border-slate-200 bg-white p-2 text-xs leading-5 dark:border-slate-800 dark:bg-slate-900">
                      {step.argument.join('\n')}
                    </pre>
                  )}
                </span>
